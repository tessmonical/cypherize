...
const createNode = async (type, options = {}) => {
  const _id = uuidv4(); // generate an id property since neo4j doesn't do that
...
const setPropertyOnNode = async (nodeOrId, options) => {
  ...
  const query = `MATCH (n) WHERE n._id=$_id SET n.${key} = n.$propValue RETURN n;`;
  const session = driver.session();
...
const setAllPropertiesOnNode = async (nodeOrId, newProperties) => {
  let _id = nodeOrId;
  if (typeof nodeOrId !== 'string') _id = nodeOrId.properties._id;

  const query = 'MATCH (n) WHERE n._id=$_id SET n=$properties RETURN n';
  const session = driver.session();
  const results = await session.run(query, { _id, properties: newProperties });
